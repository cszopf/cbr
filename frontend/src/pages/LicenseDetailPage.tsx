  ...
  return (
    <div className="space-y-1">
      ...
      <div className="w-full bg-gray-100 rounded-full h-2.5">
        <div
          className={`h-2.5 rounded-full ${isMet ? 'bg-green-500' : 'bg-blue-500'}`}
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-lg border-2 border-blue-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-900">Renewal Checklist</h2>
        <span className="text-sm text-gray-500">
          {completedCount} of {totalCount} items ready
        </span>
      </div>

      {checklist.days_until_expiration !== null && (
        <div className={`rounded-lg p-3 mb-4 ${
          checklist.days_until_expiration <= 0
            ? 'bg-red-50 border border-red-200'
            : checklist.days_until_expiration <= 30
              ? 'bg-red-50 border border-red-200'
              : checklist.days_until_expiration <= 90
          ...
          <p className={`text-sm font-medium ${
            ...
            {checklist.days_until_expiration <= 0
              ? 'License has expired! Renew immediately to avoid reactivation requirements.'
              : checklist.days_until_expiration <= 30
                ? `Urgent: ${checklist.days_until_expiration} days until expiration`
                : checklist.days_until_expiration <= 90
                  ? `${checklist.days_until_expiration} days until expiration — renewal window is open`
        ...
        {checklist.checklist.map((item, i) => (
          <div key={i} className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className={`text-sm font-medium ${item.is_complete ? 'text-green-700' : 'text-gray-900'}`}>
                {item.item}
              </p>
      ...
      >
        Renew License on Ohio eLicense Portal
      </a>
      <p className="text-xs text-gray-400 text-center mt-2">
        Opens the Ohio eLicense LPI portal in a new tab
      </p>
    </div>
  )
}
...
export default function LicenseDetailPage() {
  ...
  const { data: ceProgress } = useQuery({
    queryKey: ['ce-progress', licenseId],
    queryFn: () => getCEProgress(licenseId),
    enabled: licenseId > 0,
  })

  const { data: renewalChecklist } = useQuery({
    queryKey: ['renewal', licenseId],
    queryFn: () => getRenewalChecklist(licenseId),
    enabled: licenseId > 0 && showRenewal,
  })

  if (isLoading) return <div className="text-gray-500">Loading...</div>
  if (!license) return <div className="text-red-500">License not found</div>

  const statusColor =
    license.status === 'ACTIVE' || license.status === 'ACTIVE / PRINT LICENSE'
      ? 'bg-green-100 text-green-800'
      : license.status.startsWith('INACTIVE')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <Link to="/lookup" className="text-sm text-blue-600 hover:underline">&larr; Back to search</Link>
        <button
          onClick={() => {
        ...
        >
          {showRenewal ? 'Hide Renewal Checklist' : 'Renew License'}
        </button>
      </div>

      {showRenewal && renewalChecklist && (
        <RenewalPanel checklist={renewalChecklist} />
      )}
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-gray-500">{license.credential_type}</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
          <div>
            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-2">License Info</h3>
            <InfoRow label="License Number" value={license.license_number} />
            <InfoRow label="Type" value={license.credential_type} />
            <InfoRow label="First Issued" value={license.first_issuance_date} />
            <InfoRow label="Current License Issued" value={license.license_issued_date} />
            <InfoRow label="Expiration Date" value={license.expiration_date} />
            <InfoRow label="CE Due Date" value={license.ce_due_date} />
          </div>
          <div>
            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-2">Contact & Location</h3>
      ...
      {license.employer_name && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-3">Employer / Supervisor</h2>
      ...
      {ceProgress && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">CE Progress</h2>
            {ceProgress.ce_due_date && (
              <span className={`text-sm font-medium ${
                ceProgress.days_until_due !== null && ceProgress.days_until_due < 90
                  ? 'text-red-600'
                  : 'text-gray-500'
              }`}>
                Due: {ceProgress.ce_due_date}
                {ceProgress.days_until_due !== null && ` (${ceProgress.days_until_due} days)`}
              </span>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="w-full bg-gray-100 rounded-full h-3">
              <div
                className={`h-3 rounded-full ${ceProgress.percent_complete >= 100 ? 'bg-green-500' : 'bg-blue-500'}`}
          ...
          <div className="space-y-3">
            {ceProgress.categories.map((cat) => (
              <ProgressBar
                key={cat.category}
                label={cat.label}
                completed={cat.hours_completed}
                required={cat.hours_required}
              />
            ))}
          </div>
          ...
          {ceProgress.total_hours === 0 && (
            <p className="text-sm text-gray-400 mt-4 text-center">
              No CE records yet. Add records to track progress toward Ohio's 30-hour requirement.
            </p>
          )}
        </div>
      )}
